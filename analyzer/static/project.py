...
Dependency installation runs third-party build tooling - `forge install` runs git
hooks, `npm install` runs postinstall scripts - which is arbitrary code execution
by design. Every subprocess here goes through sandbox.run: a scrubbed
environment (no inherited secrets), kernel resource limits, its own process
group and a timeout. Callers should still run it somewhere disposable.
"""
...
def _run(command: list[str], cwd: str) -> tuple[bool, str]:
    try:
        completed = sandbox.run(command, cwd=cwd, timeout=INSTALL_TIMEOUT_SECONDS)
